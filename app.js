...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderRoster = (filter = "all") => {
    ...
    const filteredModels = window.MODELS.filter((model) => {
      if (model.division === "Kids") return false;
  ...
  if (submissionForm) {
    submissionForm.addEventListener("submit", (e) => {
      ...
      showToast(
        `Thank you, ${firstName} ${lastName}! Your model application has been submitted to Richie Careers Management. Our scouts will review your digitals.`,
      );
  ...
  if (requestForm) {
    requestForm.addEventListener("submit", (e) => {
      ...
      showToast(
        `Package inquiry sent successfully! Our Richie Careers relations managers will curate custom portfolios for ${company} within 24 hours.`,
      );
      requestForm.reset();
    });
  }
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", (e) => {
      ...
      showToast(
        `Message sent successfully! Thank you for contacting Richie Careers, ${name}.`,
      );
      contactForm.reset();
    });
  }
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", (e) => {
      ...
      showToast(
        "Thank you for subscribing to Richie Careers Newsletter! Modern modeling campaigns and editorials will be sent weekly.",
      );
